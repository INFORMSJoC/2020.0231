...
function SolveModel(A, obj, b, n)
    ...
    #-------------------------------------------------------------------------------
    function SolveP1m(A, b, x, NoS, J, trial, initx, initz)
        n = length(A[1, :, 1])
        initz = round.(initz)
        m2 = Model(solver = GurobiSolver(OutputFlag = 1, TimeLimit = 7200, ConcurrentMIP = 3, IntFeasTol = 1e-9, MIPGapAbs = 1e-5, MIPGap = 0))
        M = 2000
        epsilon = 1
    ...
    #-------------------------------------------------------------------------------
    function infnorm(xtest, xhat)
        NoS = length(xtest[:, 1])
        n = length(xtest[1,:])
        error = zeros(Float64, NoS)
        for i = 1:NoS
            error[i] = norm(xtest[i,:] - xhat[i,:], Inf)
        end
        return sum(error)/NoS
    end
    #-------------------------------------------------------------------------------
